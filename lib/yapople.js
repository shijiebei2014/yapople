...
Client.prototype.connect = function(callback) {
	...
	if (this.tls) {
		this._socket = tls.connect(this.port, this.hostname, {
            rejectUnauthorized: false
        }, function() {}.bind(this));
	} else {
		this._socket = net.createConnection(this.port, this.hostname, {
            rejectUnauthorized: false
        }, function() {}.bind(this));
	}
	this._socket.setTimeout(1000 * 60 * 2); //set timeout 2minutes
	this._socket.on('timeout', function(err) {
	...
	if (this._command.cmd === state.NOOP && this._queue.length) {
		...
		switch (this._command.cmd) {
			...
			case state.RETR:
				...
				this._write('TOP', this._command.number); break;
			case state.DELE:
